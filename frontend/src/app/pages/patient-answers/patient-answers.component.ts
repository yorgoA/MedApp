...
@Component({
  ...
  template: `
    <div class="page">
      <header class="header">
        <a routerLink="/doctor/patients" class="back">← Back</a>
      </header>
...
export class PatientAnswersComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.api.getAnswersByPatient(+id).subscribe({
      next: (data) => {
      ...
      error: () => {
        this.loading = false;
        this.errorMessage = 'Failed to load submissions';
      }
    });
  }
}
